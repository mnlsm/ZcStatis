...
OkoooEngine::OkoooEngine(const CStlString& script, const char* logf) {
	m_strScript = script;
	m_dMinBonus = 0.0;
	m_nMatchBetsLose = 0;
	m_nAvgMultiple = 0;
...
int __cdecl OkoooEngine::LUA_DbgTrace(lua_State* L) {
	if (lua_type(L, 1) == LUA_TSTRING) {
		...
		OkoooEngine* pThis = (OkoooEngine*)lua_touserdata(L, lua_upvalueindex(1));
...
int __cdecl OkoooEngine::LUA_IsFilterTJ(lua_State* L) {
	BOOL ret = FALSE;
	/*
...
BOOL OkoooEngine::CalculateAllResultImpl(CStlString& failed_reason) {
	BOOL result = FALSE;
...
lua_State* OkoooEngine::InitLua(CStlString& failed_reason) {
	...
	lua_pushcclosure(L, OkoooEngine::LUA_IsFilterTJ, 1);
	lua_setglobal(L, "IsFilterTJ");

	lua_pushlightuserdata(L, this);
	lua_pushcclosure(L, OkoooEngine::LUA_DbgTrace, 1);
...
void OkoooEngine::TermLua(lua_State* L) {
	if (L != NULL) {
		lua_close(L);
	}
}
...
BOOL OkoooEngine::GeneratorBets(const std::vector<JcBetItemSource>& arrMatchScores,
	TBetResult& result) {
...
BOOL OkoooEngine::IsAValidRecordImpl(const std::vector<JcBetItem>& record,
...
void OkoooEngine::getResults(int start, int max_count,
