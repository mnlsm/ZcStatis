...
BOOL CHttpResponseHeaders::GetCookie(const CStringATL &name, THttpResponseCookie& cookie)
{
	CStringATL nametemp = name;
	nametemp.MakeLower();
...
__int64 CHttpResponseHeaders::GetContentLength()
{
	return m_contentLength;
}

CStringATL CHttpResponseHeaders::GetRawHeaders()
{
	return m_rawHeaders;
...
void CHttpResponseHeaders::Parse( HINTERNET hRequest , const CStringATL &rawheader )
{
    ...
    bQuery = HttpQueryInfo( hRequest , HTTP_QUERY_CONTENT_LENGTH ,
							(char*)buf.data() , &len , &index );
    index = 0;
	if( bQuery )
	{
		m_contentLength = _ttoi64((TCHAR*)buf.data());
	}
    ...
    do
    {
		...
		std::string cookie = CT2A((TCHAR*)buf.data()).m_psz;
