...
BeiDanStat::BeiDanStat(const std::shared_ptr<SQLite::Database>& db) : 
	m_lstResult(this, 1),
	m_buSearch(this, 100),
	m_coIDs(this, 100),
	m_chkIDs(this, 100),
...
LRESULT BeiDanStat::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
...
LRESULT BeiDanStat::OnDestroy(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
	LRESULT lRet = CAxDialogImpl<BeiDanStat>::OnDestroy(uMsg, wParam, lParam, bHandled);
	return lRet;
}

LRESULT BeiDanStat::OnGetMinMaxInfo(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
	CRect rcDesktop;
	SystemParametersInfo(SPI_GETWORKAREA, 0, &rcDesktop, sizeof(RECT));
...
LRESULT BeiDanStat::OnQuery(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled) {
	DoQuery();
	return 1L;
}

LRESULT BeiDanStat::OnCancel(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled) {
	EndDialog(IDCANCEL);
	return 1L;
}

void BeiDanStat::InitControls() {
...
void BeiDanStat::InitData() {
	CStringATL beginDay, endDay, beginWeekDay;
	Global::getBiFenDateInfo(beginDay, endDay, beginWeekDay);
	CStringATL strSQL = (_T("SELECT ID FROM BDZQ WHERE ID LIKE ? ORDER BY ID DESC"));
...
void BeiDanStat::DoQuery() {
	...
	CStringATL strSQL;
	if (!strDate.IsEmpty()) {
		strSQL.Format(_T("SELECT ID, CATEGORY, DESCRIPTION, HAND, START_TIME, BUY_TIME, SUBJECTS, RESULT FROM BDZQ WHERE ID LIKE '%s'"), strDate);
	} else {
		strSQL = _T("SELECT ID, CATEGORY, DESCRIPTION, HAND, START_TIME, BUY_TIME, SUBJECTS, RESULT FROM BDZQ WHERE");
	}
...
LRESULT BeiDanStat::OnListRButtonDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
...
LRESULT BeiDanStat::OnSelIdsChanged(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled) {
	return 1L;
}

void BeiDanStat::DoMatchListMenuCommand(UINT cmd, UINT index) {
	if (cmd == 100) {
		CStringATL strCat;
