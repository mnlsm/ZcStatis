...
BeiDanEngine::BeiDanEngine(const CStlString& script, const char* logf) {
	m_strScript = script;
	m_dMinBonus = 0.0;
	m_nMatchBetsLose = 0;
	m_nAvgMultiple = 0;
...
int __cdecl BeiDanEngine::LUA_DbgTrace(lua_State* L) {
	if (lua_type(L, 1) == LUA_TSTRING) {
		std::string line = dbgview_prefix + lua_tostring(L, 1);
		BeiDanEngine* pThis = (BeiDanEngine*)lua_touserdata(L, lua_upvalueindex(1));
...
int __cdecl BeiDanEngine::LUA_IsFilterTJ(lua_State* L) {
	BOOL ret = FALSE;
	/*
...
BOOL BeiDanEngine::CalculateAllResult(CStlString& failed_reason) {
	failed_reason.clear();
...
void BeiDanEngine::SetSources(const std::vector<JcBetItemSource>& items) {
	m_vecSources = items;
	//调整赔率变化
...
void BeiDanEngine::SetFixedSources(const std::vector<JcBetItemSource>& items) {
	m_vecFixedSources = items;
}

BOOL BeiDanEngine::CalculateAllResultImpl(CStlString& failed_reason) {
	BOOL result = FALSE;
...
void BeiDanEngine::doFilterByBetsRankRatio(const TBetResult& allResult, const TBetResult& validResult, TBetResult& result) {
	result.clear();
...
void BeiDanEngine::doAvgMultipleResult(const TBetResult& validResult, TBetResult& result) {
	result.clear();
...
lua_State* BeiDanEngine::InitLua(CStlString& failed_reason) {
	...
	lua_pushcclosure(L, BeiDanEngine::LUA_IsFilterTJ, 1);
	lua_setglobal(L, "IsFilterTJ");

	lua_pushlightuserdata(L, this);
	lua_pushcclosure(L, BeiDanEngine::LUA_DbgTrace, 1);
...
void BeiDanEngine::TermLua(lua_State* L) {
	if (L != NULL) {
		lua_close(L);
	}
}

void BeiDanEngine::gatherMatchBets(const std::vector<JcBetItemSource>& split_scores,
...
BOOL BeiDanEngine::GeneratorBets(const std::vector<JcBetItemSource>& arrMatchScores,
	TBetResult& result) {
...
BOOL BeiDanEngine::IsAValidRecordImpl(const std::vector<JcBetItem>& record,
...
void BeiDanEngine::push_scriptfunc_params(lua_State* L, const std::vector<JcBetItem>& record) {
	lua_newtable(L);
	double betbouns = 0.0;
...
void BeiDanEngine::getAllMatchIds(CStlStrArray& matchIds) {
	matchIds.clear();
...
void BeiDanEngine::getResults(int start, int max_count,
