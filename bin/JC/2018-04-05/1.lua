...
kMatchDesc = "混合过滤";
kMatchBets = {
    "20180405001;-1;6,3,1.24;1,3,1.86",         --         阿森纳   VS   莫斯科中央陆军
    "20180405002;-1;6,3,1.22;1,3,1.88",         --   马德里竞技   VS   里斯本竞技   
    "20180405005;-1;3,1,5.80;3,2,6.00;3,3,7.00;3,4,9.00",         --      科洛科洛   VS   曼塔海豚      
    "20180405006;-1;3,1,5.10;3,2,5.30;3,3,7.00;3,4,8.00",         --            河床   VS   圣菲独立      
};
...
function IsFilterLua(params)
	...
	ok_count = 0;
	ok_count = GetMatchResultCount(6, 3, codes);
	...
	ok_count = 0;
	ok_count = GetMatchResultCount(3, 1, codes);
	ret.info = trace_prefix .. "ok_count_1=" .. (ok_count);
	trace(1, ret.info);
	if(ok_count > 1) then
		ret.code = 1;
	...
	ok_count = 0;
	ok_count = GetMatchResultCount(3, 4, codes);
	ret.info = trace_prefix .. "ok_count_1=" .. (ok_count);
	trace(1, ret.info);
	if(ok_count > 1) then
		ret.code = 1;
