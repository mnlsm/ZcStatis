...
LRESULT ZuCaiDialog::OnBetAreaLButtonDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
...
LRESULT ZuCaiDialog::OnBetAreaEraseBkgnd(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
	return TRUE;
}

LRESULT ZuCaiDialog::OnBetAreaPaint(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
...
void ZuCaiDialog::DrawBetAreaTitle(CDCHandle dc, const CRect& rcc, int& yTop) {
...
void ZuCaiDialog::DrawSPF(CDCHandle dc, const CRect& rcc, int& yTop) {
	int saveDC = dc.SaveDC();
	dc.SetBkMode(TRANSPARENT);
	int itemWidth = 80;
	int itemHeight = 40;
		...
		for (int i = 0; i < 6; i++) {
			...
			if (subject != NULL) {
				if (subject->checked) {
					...
					if (m_CurrentMatchItem->multi_selected) {
						dc.FillSolidRect(rcItem, RGB(0x00, 0x7F, 0x00));
					}
...
void ZuCaiDialog::DrawJQZS(CDCHandle dc, const CRect& rcc, int& yTop) {
	int saveDC = dc.SaveDC();
	dc.SetBkMode(TRANSPARENT);
	int itemWidth = 80;
	int itemHeight = 40;
...
void ZuCaiDialog::DrawBQC(CDCHandle dc, const CRect& rcc, int& yTop) {
	int saveDC = dc.SaveDC();
	dc.SetBkMode(TRANSPARENT);
	int itemWidth = 80;
	int itemHeight = 40;
...
void ZuCaiDialog::DrawBF(CDCHandle dc, const CRect& rcc, int& yTop) {
	int saveDC = dc.SaveDC();
	dc.SetBkMode(TRANSPARENT);
	int itemWidth = 60;
	int itemHeight = 40;
